...
public class MainActivity extends AppCompatActivity {
  ...
  public static SharedPreferences.Editor writeData;
    ...
    private void serverConnect(){
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                String resultMarkers = array_result[0];
                Log.d(LOG, "Ответ сервера " + resultMarkers.toString());
                ...
                try {
                    ...
                    jsonObject = new JSONObject(resultMarkers);
                    JSONArray cityMarkers = jsonObject.getJSONArray(cityName+"Markers");

                   for (int i = 0; i < cityMarkers.length(); i++) {
                        //Следущие дынные будут писаться в базу SQLite
                        String name = cityMarkers.getJSONObject(i).getString("name");               //название достопримечательности
                        String type = cityMarkers.getJSONObject(i).getString("type");               //тип достопримечательности
                        String lat = cityMarkers.getJSONObject(i).getString("lat");                 //широта
                        String lon = cityMarkers.getJSONObject(i).getString("lon");                 //долгота
                        String descript = cityMarkers.getJSONObject(i).getString("descript");       //описание
                        String image = cityMarkers.getJSONObject(i).getString("image");             //путь к картинке
                        String video = cityMarkers.getJSONObject(i).getString("video");             //путь к видео
                        int count_people = cityMarkers.getJSONObject(i).getInt("count_people");     //количество проголосовавших людей
                        int summ_mark = cityMarkers.getJSONObject(i).getInt("summ_mark");           //общая сумма голосов
                        String comment = cityMarkers.getJSONObject(i).getString("comment");         //комментарии
                        ...
                        long rowID = db.insert(cityName+"Markers", null, dbValue);
